 ...
 * @package SasCMS
 * @author Spazzy
 * @see https://twitter.com/spazzyy
 * @warning PRIVATE CODE. DO NOT SHARE, EVERY TEMPLATE IS UNIQUE, IF FOUND YOU DID'NT ABIDE BY THIS, YOU KNOW.
 */
...
class Sas
{
	...
	/** @var string SAS_REPO - repo URL */
	public const SAS_REPO = 'https://raw.githubusercontent.com/robiso/wondercms/master/';

	/** @var string SAS_CDN_REPO - CDN repo URL */
	public const SAS_CDN_REPO = 'https://raw.githubusercontent.com/robiso/wondercms-cdn-files/master/';
	...
	/**
	 * Backup whole sasCMS installation
	 *
	 * @return void
	 * @throws Exception
	 */
	...
	public function betterSecurityAction(): void
	{
		if (isset($_POST['betterSecurity']) && $this->verifyFormActions()) {
			if ($_POST['betterSecurity'] === 'on') {
				if ($contents = $this->getFileFromRepo('htaccess-ultimate', self::SAS_CDN_REPO)) {
					file_put_contents('.htaccess', trim($contents));
				}
				$this->alert('success', 'Improved security turned ON.');
				$this->redirect();
			} elseif ($_POST['betterSecurity'] === 'off') {
				if ($contents = $this->getFileFromRepo('htaccess', self::SAS_CDN_REPO)) {
					file_put_contents('.htaccess', trim($contents));
				}
	...
	/**
	 * Check if we can run sasCMS properly
		...
		$this->db = (object)[
			...
			'pages' => [
				...
				'home' => [
					...
					'content' => '<h1>Market Is Alive!</h1>

<h4><a href="' . self::url('loginURL') . '">Click to login into the admin area.</a> Your password is: <b>' . $password . '</b></a></h4>'
				],
				'example' => [
					'title' => 'Example',
					'keywords' => 'Keywords, are, good, for, search, engines',
...
EOT;
	 ...
	 * @return string
	 */
	public function getFileFromRepo(string $file, string $repo = self::SAS_REPO): string
	{
	 ...
	 * @return null|string
	 */
	public function getOfficialVersion(string $repo = self::SAS_REPO): ?string
	{
	 ...
	 * @return null|string
	 */
	public function getCheckFileFromRepo(string $fileName, string $repo = self::SAS_REPO): ?string
	{
	...
	{
		$plugins = trim($this->getFileFromRepo('plugins-list.json', self::SAS_CDN_REPO));
		$themes = trim($this->getFileFromRepo('themes-list.json', self::SAS_CDN_REPO));
...
EOT;
	...
	private function checkWcmsCoreUpdate(): void
	{
		...
		if ($onlineVersion > VERSION) {
			$this->alert(
				'info',
				'<h4><b>New sasCMS update available</b></h4> - Backup your website and
				<a href="https://" target="_blank"><u>check what\'s new</u></a> before updating.
				 <form action="' . self::url($this->currentPage) . '" method="post" class="marginTop5">
					...
					<div class="clear"></div>
					<button class="btn btn-info marginTop5" name="update">Update sasCMS ' . VERSION . ' to ' . $onlineVersion . '</button>
	...
	public function settings(): string
	{
		...
		$output .= '		</div>
					...
					<div class="modal-footer clear">
						<p class="small">
							<a href="https://wondercms.com" target="_blank">sasCMS ' . VERSION . '</a> &nbsp;
	...
	public function updateAction(): void
	{
		...
		if ($contents) {
			...
			$this->alert('success', 'sasCMS successfully updated. Wohoo!');
			$this->redirect();
		}
		$this->alert('danger', 'Something went wrong. Could not update sasCMS.');
		$this->redirect();
	}
	...
	/**
	 * Create a ZIP backup of whole sasCMS installation (all files)
	 *
	 * @return void
	 */
	...
	private function checkMinimumRequirements(): void
	{
		if (PHP_VERSION_ID <= 70200) {
			die('<p>To run sasCMS, PHP version 7.2 or greater is required.</p>');
		}
